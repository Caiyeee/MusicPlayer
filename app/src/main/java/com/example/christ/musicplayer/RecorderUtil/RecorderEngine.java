...
//            CommonFunction.showToast(mContext, "无法录制语音，请检查您的手机存储", "VoiceRecorder");
            Log.e("VoiceRecorder", "无法录制语音，请检查您的手机存储");
            return false;
        }
        ...
        if (!recordFile.exists()) {
            try {
                ...
                Log.e("建立语音文件异常:" + recordFileUrl, e.toString());

//                CommonFunction.showToast(mContext, "建立语音文件异常", "VoiceRecorder");
                return false;
            }
        }
    ...
    public void startRecordVoice(Context mContext, String recordFileUrl,
                                 VoiceRecorderOperateInterface voiceRecorderOperateInterface) {
        stopRecordVoice(mContext);
        ...
        if (recording) {
            ...
            CommonFunction.showToast(mContext, "录音失败", "VoiceRecorder");
            Log.e("VoiceRecorder", "录音失败");
    ...
    public void stopRecordVoice(Context mContext) {
        if (recording) {
            ...
            if (!recordVoiceSuccess) {
                CommonFunction.showToast(mContext, "录音太短", "VoiceRecorder");

                if (voiceRecorderInterface != null) {
                    voiceRecorderInterface.recordVoiceFail();
                }

                FileFunction.DeleteFile(mContext, recordFileUrl);
                return;
            }
    ...
    public void giveUpRecordVoice(Context mContext, boolean fromHand) {
        if (recording) {
